...
import { toGameStateView } from '@shared/lib/to-game-state-view'
...
export default function App() {
  ...
  const [winBannerCollapsed, setWinBannerCollapsed] = useState(false)
  ...
  const revealWord = (position: number[]) => {
    ...
    setGame(produce(game, (draft) => {
      ...
      if (response === 'ok') setRevealingWord(false)
      else setGuessedWordError(errorMessages[response] || 'Error desconocido.')
    }))
  }
  ...
  const state = toGameStateView(game, { gameEnded, gameWon })
      ...
      <div className="flex flex-col gap-4 unselectable pt-3 w-full max-w-2xl relative z-10">
        <div className={clsx({ 'blur-sm opacity-50 pointer-events-none transition-all duration-300': revealingWord })}>
          ...
          {(gameWon || gameEnded) && (
            ...
            >
              <div className={clsx('text-2xl font-bold mb-2', gameWon ? 'text-green-600' : 'text-red-600')}>
                {gameWon
                  ? progress === 100
        ...
        <div className={clsx({ 'blur-sm opacity-50 pointer-events-none transition-all duration-300': revealingWord, 'flex flex-col gap-4': true })}>
          ...
          <div className="flex justify-center gap-4 mt-2">
            <RevealLetterInput revealedLetters={game.revealedLetters} revealLetterHandler={revealLetter} />
            <RevealWordInput revealedWords={game.revealedWords} revealWordHandler={startRevealWord} />
          </div>
        </div>
      </div>
    </div>
